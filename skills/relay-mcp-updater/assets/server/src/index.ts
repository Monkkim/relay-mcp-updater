...
import { RelayOAuthProvider, handleLogin, loginCookieName, LOGIN_PATH, RESET_PATH } from "./auth/provider.js";
...
async function main() {
  ...
  app.use((req, _res, next) => {
    console.log(`[req] ${req.method} ${req.path} ua=${(req.headers["user-agent"] || "").slice(0, 60)}`);
    next();
  });
  ...
  app.get("/assets/pocketbase.es.mjs", (_req, res) => {
    res.sendFile(path.resolve("node_modules/pocketbase/dist/pocketbase.es.mjs"));
  });
  app.get("/assets/relay-login.js", (_req, res) => {
    res.set("Cache-Control", "no-store");
    res.sendFile(path.resolve("src/public/relay-login.js"));
  });
  app.get("/assets/auth-methods.js", (_req, res) => {
    res.sendFile(path.resolve("src/public/auth-methods.js"));
  });

  app.post(LOGIN_PATH, express.urlencoded({ extended: false, limit: "20kb" }), async (req, res) => {
    res.set("Cache-Control", "no-store");
    const state = typeof req.body.state === "string" ? req.body.state : "";
    const pbToken = typeof req.body.pbToken === "string" ? req.body.pbToken : "";
    const validState = /^[a-f0-9]{48}$/.test(state);
    const cookieName = validState ? loginCookieName(state) : "";
    const browserBound = validState && req.get("origin") === new URL(PUBLIC_URL).origin &&
      (req.headers.cookie || "").split(";").some(part => part.trim() === `${cookieName}=1`);
    const result = await handleLogin({ state, pbToken, browserBound });
    if ("redirectUrl" in result) {
      res.clearCookie(cookieName, {path: "/", secure: PUBLIC_URL.startsWith("https:"), httpOnly: true, sameSite: "lax"});
  ...
  app.post(RESET_PATH, (_req, res) => {
    res.status(410).type("text/plain").send("Password sign-in is no longer available. Restart the connection and choose your Relay sign-in provider.");
  });
  ...
  app.get("/health", (_req, res) => {
    res.json({ status: "ok", version: "2.0.1", build: BUILD_IDENTITY, sessions: sessions.size });
  });
